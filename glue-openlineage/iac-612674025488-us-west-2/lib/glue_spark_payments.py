...
def main():
    ...
    input_path = "s3://lh-core-kolya-landing-zone/files/raw_payments.csv"
    ...
    # payments_df.writeTo(output_table) \
    #     .option("write.format.default", "parquet") \
    #     .option("write.parquet.compression-codec", "snappy") \
    #     .createOrReplace()

    # DataFrameWriterV1 API
    payments_df.write.format("iceberg").option(
        "path", "s3://lh-core-kolya-landing-zone/icebergs/demo.db/payments"
    ).mode("overwrite").saveAsTable("glue_catalog.demo_db.payments")
